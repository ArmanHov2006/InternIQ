...
export async function POST(request: Request) {
  try {
    ...
    if (authError) {
      const isMissingSession = authError.message.toLowerCase().includes("auth session missing");
    ...
    if (!body.job_url?.trim() || !body.resume_text?.trim()) {
      return NextResponse.json(
        { error: "job_url and resume_text are required." },
        { status: 400 }
      );
    }
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: parseError(payload, "Failed to analyze fit.") },
